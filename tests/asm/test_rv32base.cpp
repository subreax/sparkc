...
TEST_CASE("Test core risc-v instructions", "[riscv-core]") {
    ...
    REQUIRE( Rv32Base::jType(0b1010101, RvReg::A0) | Rv32Base::encodeImmJ(0b001010101110101010100) == _0101 );

    // imm:10101010101010101010 rd:10101 op:0101010
    // 2^21 - 0b110101010001010101010
    REQUIRE( Rv32Base::jType(0b0101010, RvReg::S5) | Rv32Base::encodeImmJ(-351574) == _1010 );

    // 1 0000000000 1 00000000 11111 0000000
    REQUIRE( Rv32Base::jType(0, (RvReg) 0b11111) | Rv32Base::encodeImmJ(-1046528) == 0b10000000000100000000111110000000u );
