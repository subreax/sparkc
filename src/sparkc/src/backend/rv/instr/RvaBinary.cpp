...
RvaBinary::RvaBinary(
    RvaValue* dst, RvaValue* left, Operator op, RvaValue* right)
    : RvaInstruction(Kind::Binary), dst(dst), left(left), op(op), right(right) {
}
...
void RvaBinary::emit(RvListing& listing) {
    ...
    case Operator::Plus:
        ...
        if (right->kind == RvaValue::Kind::Imm) {
            listing +=
                Rv32I::addi(expectReg(dst), expectReg(left), expectImm(right));
        }
        else {
            listing +=
                Rv32I::add(expectReg(dst), expectReg(left), expectReg(right));
        }
        break;
    ...
    case Operator::Minus:
        // todo: support left or right as imm
        listing +=
            Rv32I::sub(expectReg(dst), expectReg(left), expectReg(right));
        break;

    case Operator::Mul:
        listing +=
            Rv32M::mul(expectReg(dst), expectReg(left), expectReg(right));
        break;

    case Operator::MulH:
        listing +=
            Rv32M::mulh(expectReg(dst), expectReg(left), expectReg(right));
        break;

    case Operator::Div:
        listing +=
            Rv32M::div(expectReg(dst), expectReg(left), expectReg(right));
        break;

    case Operator::Rem:
        listing +=
            Rv32M::rem(expectReg(dst), expectReg(left), expectReg(right));
        break;
    ...
    case Operator::LessThan:
        // todo: support left or right as imm
        listing +=
            Rv32I::slt(expectReg(dst), expectReg(left), expectReg(right));
        break;
    ...
    case Operator::LessOrEqual:
        ...
        // (left > right) == 0
        listing +=
            Rv32I::sgt(expectReg(dst), expectReg(left), expectReg(right));
        listing += Rv32I::seqz(expectReg(dst), expectReg(dst));
        break;
    ...
    case Operator::ShiftLeft:
        listing +=
            Rv32I::slli(expectReg(dst), expectReg(left), expectImm(right));
        break;

    case Operator::ShiftRight:
        listing +=
            Rv32I::srli(expectReg(dst), expectReg(left), expectImm(right));
        break;

    case Operator::Or:
        if (right->kind == RvaValue::Kind::Imm) {
            listing +=
                Rv32I::ori(expectReg(dst), expectReg(left), expectImm(right));
        }
        else {
            listing +=
                Rv32I::or_(expectReg(dst), expectReg(left), expectReg(right));
        }
        break;
